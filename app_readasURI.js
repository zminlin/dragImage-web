			...
			utils.addListener(el, 'drop' ,function (e) {
				...
				**/
				...
				for(var i=0; i<fileList.length;i++){
					...
					utils.addListener(reader, 'load', imageLoader);
					fileList[i].id = deliverSer.getID();
					reader.index = deliverSer.getID();
					console.log(fileList[i]);
					pics.push(fileList[i]);		
					deliverSer.addPics();//upload pic	
...
/*
.factory('photoReader', [function () {
	var factory=[];
	factory.loadPhoto = function (fileList) {
		for(var i=0; i<fileList.length;i++){
			var reader = new FileReader();
			utils.addListener(reader, 'load', imageLoader);
			fileList[i].id = deliverSer.getID();
		...
		function imageLoader(e) {
			console.log(e);
			var img= document.getElementById('photo_view'+e.target.index);					
			img.src = e.target.result;
			//
